...
function createChildComment(container, comment) {
  ...
  const timestamp = convertMillisToTimestamp(comment.timestamp);
  div.innerHTML = `
    <article class="media">  
...
function drawGauge(commentList) {
  const rating = getGaugeValue(commentList);
  ...
  const chart =
      new google.visualization.Gauge(document.getElementById('gauge-chart'));

  chart.draw(data, options);
}

function updateMaxComments() {
  const val = document.getElementById('input-max-comments').value;
